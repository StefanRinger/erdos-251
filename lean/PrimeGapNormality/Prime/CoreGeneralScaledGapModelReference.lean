...
/-!
# Arbitrary calibrated mixtures for deterministically scaled gap observables

Only the deterministic CompactScaling property is imported from the
rounded route. The original law is the actual arbitrary cutoff mixture.
Its full survivor count is retained, the original cutoff is charged before
resampling, and the outer phase uses the universal slot. The canonical
sqrt reserve is unchanged when the critical selector uses G^a.
-/

namespace PrimeGapNormality.Prime.CoreGeneralScaledGapModelReference
...
def scale (B : ℕ) (a : ℝ) (G : ℕ → ℝ) (J : ℕ → ℕ) (X : ℕ) : ℝ :=
  G X ^ a / (B : ℝ) ^ (J X + 1)

def modelMean (B : ℕ) {k : ℕ} (hk : 0 < k) (r : Fin k) (F : Fin k → ℕ → ℤ)
    (κ : ℝ) (G : ℕ → ℝ) (ω : ℕ → ℕ → ℝ) (X : ℕ) (f : Circle →ᵇ ℝ) : ℝ :=
  CoreCalibratedModelFinite.mean (ω X) (physicalSpan G (rank κ G) X) (rank κ G X)
    (actualTest B hk r F (rank κ G X) f)

def modelLaw (B : ℕ) {k : ℕ} (hk : 0 < k) (r : Fin k) (F : Fin k → ℕ → ℤ)
    (a κ : ℝ) (G : ℕ → ℝ) (ω : ℕ → ℕ → ℝ) (J : ℕ → ℕ) (X : ℕ) : ProbabilityMeasure Joint :=
  profileLaw G (rank κ G) ω (fun n => OnePoint.FiniteExterior.frameRank 0 (J n))
    (scale B a G J) (fun n => outerMap B hk r F (rank κ G n) (J n) (physicalSpan G (rank κ G) n)) X

private theorem single_cutoff_bound (S y M J : ℕ) (hSy : S ≤ y)
    (hJ : 1 ≤ J) (hJM : J < M) {G R δ : ℝ} (hG : 1 < G) (hR : 0 < R) (hδ : 0 ≤ δ)
    (hθ : 0 < lateRetention S y) (hθ4 : lateRetention S y ≤ (1 : ℝ) / 4)
    (hret : lateRetention S y * G / Real.log G ≤ 6 / eulerProdLowerConst)
    ...
    CoreOriginalDeletedFrame.highMean y S M test ≤ modelConstant *
  ...
  have hm := layer_contraction S y M hSy hG
    (div_nonneg (by norm_num) eulerProdLowerConst_pos.le) hδ hret v hv
  ...
  exact hh.trans_eq (by unfold modelConstant; ring)
...
theorem eventually_mixture
    ...
    ∀ᶠ G : ℝ in atTop, ∀ (J S M : ℕ) (ω : ℕ → ℝ), Weights ω S →
      ...
      CoreCalibratedModelFinite.mean ω S M (actualTest B hk r F M f) ≤
        modelConstant * ((∫ z, compactTest hR.le ha ((B : ℝ) * (b s : ℝ))
  ...
  have hp (y : ℕ) : ω y * CoreOriginalDeletedFrame.highMean y S M (actualTest B hk r F M f) ≤
      ω y * (modelConstant * ((∑ E ∈ (offsetWindow S).powerset, coreAuxiliaryLayerFrameMass S y M E * v E) + δ) +
...
theorem exists_label_ranks {B k : ℕ} (hB : 2 ≤ B) (hk : 0 < k) (r : Fin k)
    {a κ : ℝ} (ha : 0 < a) (ha1 : a ≤ 1) (b : Fin k → ℤ) (hb : b ≠ 0)
    (hκ : 1 / Real.log (B : ℝ) ≤ κ) {G : ℕ → ℝ} (hG : Tendsto G atTop atTop) :
    ∃ s : Fin k, CoreRoundedCriticalRank.UsableLeadingLabel B hk a b s ∧
      ∃ J : ℕ → ℕ, ∀ᶠ X in atTop,
        1 ≤ J X ∧ J X < rank κ G X ∧ phaseAt hk r (J X - 1) = s ∧
          1 ≤ scale B a G J X ∧ scale B a G J X < (B : ℝ) ^ k := by
  obtain ⟨s, hs⟩ := CoreRoundedCriticalRank.exists_usableLeadingLabel hB hk ha ha1 b hb
  obtain ⟨c, hc, hlabel⟩ := exists_insertion_residue hk r s
  let p (X j : ℕ) : Prop := 1 ≤ j ∧ j < rank κ G X ∧ phaseAt hk r (j - 1) = s ∧
    1 ≤ G X ^ a / (B : ℝ) ^ (j + 1) ∧ G X ^ a / (B : ℝ) ^ (j + 1) < (B : ℝ) ^ k
  have he : ∀ᶠ X in atTop, ∃ j, p X j := by
    filter_upwards [hG.eventually (CoreRoundedCriticalRank.eventually_realCriticalRank hB hk hc ha ha1 hκ)] with X hx
    obtain ⟨j, hj, hjL, hmod, hlo, hhi⟩ := hx
    exact ⟨j, hj, hjL, hlabel j hj hmod, hlo, hhi⟩
  let J X := if h : ∃ j, p X j then Classical.choose h else 0
  refine ⟨s, hs, J, ?_⟩
  filter_upwards [he] with X hx
  have hh : p X (J X) := by dsimp only [J]; rw [dif_pos hx]; exact Classical.choose_spec hx
  exact hh

/-- Final actual model reference for arbitrary calibrated mixtures. No
linearity of F, independence, full-line image AC, or marked-index law is
assumed. CompactScaling is purely deterministic and is discharged by the
rounded-power/rounded-polynomial constructions downstream. -/
theorem exists_reference_subsequence
    ...
    ∃ ψ : ℕ → ℕ, StrictMono ψ ∧ ∃ σ : Measure Circle,
      ...
      ∀ (f : Circle →ᵇ ℝ) (Kf : ℝ≥0), LipschitzWith Kf f → (∀ x, 0 ≤ f x) →
        ∀ ε : ℝ, 0 < ε → ∀ᶠ n in atTop,
          modelMean B hk r F κ G ω (ψ n) f ≤ modelConstant * (∫ x, f x ∂σ) + ε := by
  ...
  have hmass : σ Set.univ ≤ ENNReal.ofReal 4 := by
    change CoreRoundedReferenceMeasure.referenceMeasure ν a A C Set.univ ≤ ENNReal.ofReal 4
  ...
  apply CoreRoundedModelSubsequence.reference_domination_of_finite_bounds ha
    ...
    (fun n f => modelMean B hk r F κ G ω (ψ n) f) modelConstant_nonneg
  ...
  have hfinite : ∀ᶠ X : ℕ in atTop,
    ...
    have hp := hm (J X) (physicalSpan G (rank κ G) X) (rank κ G X) (ω X) hwgt
      hj.1 hj.2.1 hj.2.2.1 hret.1 hret.2.2.1 (by
        ...
        exact ⟨hh.2.1, hh.2.2.1, hh.2.2.2, hdata.2.2.1 y hy⟩) hj.2.2.2.1 hj.2.2.2.2
...
end PrimeGapNormality.Prime.CoreGeneralScaledGapModelReference
