...
/-!
# Actual relations under positive arbitrary-mixture S/T

The existing sequence-generic linear map supplies exact finite/tsum
linearity. Its convergence is derived from literal positive S and bare T.
...
-/
namespace PrimeGapNormality.Prime.CoreGeneralPositiveRelations
...
theorem fullSeries_sum_smul
  ...
  let ev := CoreSequenceSTRelations.sequenceSeriesLinear hB hk phase
    (gap_growth ha hκp hc hG hω hsum hcal hS hT)
...
theorem combination_weyl
  ...
  have hW := local_weyl ha hB hk phase (∑ i, q i • F i)
    (by rwa [CoreLocalRelations.normalForm_sum_smul]) hκp hκq hc hG hω hsum hcal hS hT
  rwa [fullSeries_sum_smul ha hB hk phase F q hκp hc hG hω hsum hcal hS hT] at hW
...
theorem rational_relation_iff
  ...
  have hh := local_rational_iff_normalForm_zero ha hB hk phase (∑ i, q i • F i)
    hκp hκq hc hG hω hsum hcal hS hT
  rwa [fullSeries_sum_smul ha hB hk phase F q hκp hc hG hω hsum hcal hS hT,
...
theorem jointWeyl_of_independent_normalForms
  ...
  have hW := combination_weyl ha hB hk phase F hdeg hκp hκ hc hG hω hsum hcal hS hT
    (fun i => (t i : ℚ)) hq
...
theorem one_linearIndependent
  ...
  convert (linearIndependent_one_jointWeyl (I := I) hclock
    (jointWeyl_of_independent_normalForms ha hB hk phase F hdeg hlin hκp hκ hc hG hω hsum hcal hS hT)) using 1 <;>
    first | (funext i; cases i <;> rfl) | rfl
...
theorem joint_empirical_tendsto
  ...
  CoreJointEquidistribution.empirical_tendsto_of_jointWeyl
    (jointWeyl_of_independent_normalForms ha hB hk phase F hdeg hlin hκp hκ hc hG hω hsum hcal hS hT)
...
theorem jointWeyl_common_period
  ...
  have hh := jointWeyl_of_independent_normalForms ha hB hl ⟨0, hl⟩
    (CoreCommonPeriodEnd.family hk l F) hgdeg hlin' hκp hκ hc hG hω hsum hcal hS hT
...
theorem jointWeyl_realCutoff
    ...
    (hT : MeanGapTailT a (localTailBase κ) G) :
    JointWeyl (fun _ : I => B ^ k)
      (fun i => coreCyclicFullSeries B hk phase (fun n => (seqGap a n : ℝ)) (F i)) :=
  jointWeyl_of_independent_normalForms ha hB hk phase F hdeg hlin hκp hκ hc hG
...
theorem rational_relation_iff_realCutoff
    {I : Type*} [Fintype I] {a : ℕ → ℕ} (ha : StrictMono a)
    {B k d : ℕ} (hB : 2 ≤ B) (hk : 0 < k) (phase : Fin k)
    (F : I → PeriodicLocal k) (hdeg : ∀ i, topDegree (normalForm hB hk (F i)) ≤ d)
    {κ : ℝ} (hκp : 0 < κ) (hκ : (d : ℝ) / Real.log (B : ℝ) ≤ κ)
    ...
    (∃ z : ℚ, (∑ i, (q i : ℝ) *
      coreCyclicFullSeries B hk phase (fun n => (seqGap a n : ℝ)) (F i)) = (z : ℝ)) ↔
      (∑ i, q i • normalForm hB hk (F i)) = 0 :=
  rational_relation_iff ha hB hk phase F hdeg hκp hκ hc hG
    (fun X y => CoreCalibratedRealCutoff.real_weights_nonneg (μ X) y)
    (fun X => CoreCalibratedRealCutoff.real_weights_sum (μ X))
    (CoreCalibratedRealCutoff.uniformCalibration_of_ae μ G hcal)
    (CoreGeneralRoundedPositive.shapeS_of_real hG hcal hS) hT q

end
end PrimeGapNormality.Prime.CoreGeneralPositiveRelations
