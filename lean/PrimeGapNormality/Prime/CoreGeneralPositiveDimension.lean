import PrimeGapNormality.Prime.CoreGeneralPositiveRelations
import PrimeGapNormality.Prime.CoreSequenceSTDimension
import PrimeGapNormality.Prime.CoreGeneralRealCutoff

/-!
# Actual series-span dimension under positive arbitrary-mixture S/T
...
-/
namespace PrimeGapNormality.Prime.CoreGeneralPositiveDimension
...
theorem evaluation_injective
  ...
  have hker : ∀ v, CoreSequenceSTDimension.evaluation hB hk phase hH D hg v = 0 → v = 0 := by
    ...
    have hNF := (CoreGeneralPositiveClassification.local_rational_iff_normalForm_zero
      ha hB hk phase (embedRooted hH D k F) hκp hκF hc hG hω hsum hcal hS hT).1
...
actual series, under the same positive S, bare T and qualitative calibration. -/
theorem finrank_seriesSpan
  ...
  have hg := gap_growth ha hκp hc hG hω hsum hcal hS hT
  rw [← CoreSequenceSTDimension.evaluation_range_eq_seriesSpan hB hk phase hH hg,
    LinearMap.finrank_range_of_inj
      (evaluation_injective ha hB hk phase hH hg hκp hκ hc hG hω hsum hcal hS hT)]
...
theorem finrank_seriesSpan_realCutoff
    ...
    (hB : 2 ≤ B) (hk : 0 < k) (phase : Fin k) (hH : 0 < H) (hD : 0 < D)
    {κ : ℝ} (hκ : (D : ℝ) / Real.log (B : ℝ) ≤ κ)
    {c : ℝ} (hc : 0 < c) {G : ℕ → ℝ} (hG : Tendsto G atTop atTop) (μ : ℕ → ProbabilityMeasure ℝ)
    (hcal : CoreGeneralRealCutoff.Calibration μ G) (hS : CoreGeneralRoundedPositive.RealShapeS a κ G μ c)
    (hT : MeanGapTailT a (localTailBase κ) G) :
    Module.finrank ℚ (CoreSequenceSTDimension.seriesSpan a B H D hk phase) =
      1 + k * (H + D - 1).choose (D - 1) :=
  finrank_seriesSpan ha hB hk phase hH hD hκ hc hG
...
end PrimeGapNormality.Prime.CoreGeneralPositiveDimension
