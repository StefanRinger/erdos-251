/-
Vendored source candidate from https://github.com/alexKontorovich/PrimeNumberTheoremAnd/blob/a5154676af9aa3095150ee410cdda80555aa0642/PrimeNumberTheoremAnd/Fourier.lean
Pinned commit: a5154676af9aa3095150ee410cdda80555aa0642 (2026-08-30). Upstream license: Apache-2.0.
The sole local import is rewritten to the relocated Sobolev module.
...
import PrimeGapNormality.ClassicalPNT.Sobolev
...
theorem nnnorm_eq_of_mem_circle (z : Circle) : ‖z.val‖₊ = 1 := by
  apply NNReal.eq
  simpa only [coe_nnnorm, NNReal.coe_one] using Circle.norm_coe z
...
noncomputable def e (u : ℝ) : ℝ →ᵇ ℂ where
  ...
  map_bounded' :=
    ⟨2, fun x y => (dist_le_norm_add_norm _ _).trans (by
      rw [Circle.norm_coe, Circle.norm_coe]
      norm_num)⟩
