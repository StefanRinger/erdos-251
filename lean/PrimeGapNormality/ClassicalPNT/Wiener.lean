/-
Vendored source candidate from https://github.com/alexKontorovich/PrimeNumberTheoremAnd/blob/a5154676af9aa3095150ee410cdda80555aa0642/PrimeNumberTheoremAnd/Wiener.lean
Pinned commit: a5154676af9aa3095150ee410cdda80555aa0642 (2026-08-30). Upstream license: Apache-2.0.
Extracted lines 1-2461 through the complete WeakPNT proof. Removed Architect and all non-semantic blueprint metadata. Removed the unused alternative-decay declarations prelim_decay_2, AbsolutelyContinuous, prelim_decay_3, and decay_alt, plus their now-unused BoundedVariation import. No declaration on the textual WeakPNT dependency path was removed.
...
import PrimeGapNormality.ClassicalPNT.Mathlib.Analysis.Asymptotics.Asymptotics
import PrimeGapNormality.ClassicalPNT.Fourier
import PrimeGapNormality.ClassicalPNT.SmoothExistence
...
lemma second_fourier_integrable_aux1 (hcont : Measurable ψ) (hsupp : Integrable ψ) (hσ : 1 < σ') :
  ...
  · apply Measurable.aestronglyMeasurable
    ...
    simp only [neg_mul, ofReal_exp, ofReal_neg, ofReal_mul, ofReal_sub, ofReal_one,
      smul_eq_mul, Function.uncurry]
    refine Measurable.mul ?_ (Measurable.mul ?_ (hcont.comp measurable_snd))
    · exact ((measurable_ofReal.comp measurable_fst).mul measurable_const).neg.cexp
    · have harg : Continuous fun p : ℝ × ℝ ↦ -(p.2 * (p.1 / (2 * π))) :=
        (continuous_snd.mul (continuous_fst.div_const (2 * π))).neg
      exact (continuous_induced_dom.comp
        (continuous_fourierChar.comp (continuous_multiplicative_ofAdd.comp harg))).measurable
...
theorem sum_le_integral {x₀ : ℝ} {f : ℝ → ℝ} {n : ℕ} (hf : AntitoneOn f (Ioc x₀ (x₀ + n)))
  ...
  have l1 : f (x₀ + 1) ≤ ∫ x in x₀..x₀ + 1, f x := by
    ...
    rw [mem_ae_iff, Measure.restrict_apply (measurableSet_Ioc.compl)]
    simp [this, measure_singleton]
